...
import { MUSIC_TRACKS, MusicTrack, getTrackById } from '@/lib/music-tracks';
...
function CreateEventFormContent() {
  ...
  const { getEvent } = useAppContext();

  const [fetchedEvent, setFetchedEvent] = useState<any>(undefined);
  const initialEvent = editId ? (getEvent(editId) || fetchedEvent) : undefined;

  const [selectedTheme, setSelectedTheme]  = useState(THEMES[0]);
  const [selectedTrack, setSelectedTrack]  = useState<MusicTrack>(MUSIC_TRACKS[1]);
  const [playingId, setPlayingId]          = useState<string | null>(null);
  const audioRef                           = useRef<HTMLAudioElement | null>(null);

  useEffect(() => {
    if (editId && !getEvent(editId)) {
  ...
  useEffect(() => {
    if (initialEvent) {
      if (initialEvent.theme) {
        const found = THEMES.find((t) => t.name === initialEvent.theme?.name || t.primary === initialEvent.theme?.primary);
        if (found) setSelectedTheme(found);
        else setSelectedTheme({
          name: initialEvent.theme.name || 'Custom',
          nameAr: initialEvent.theme.name || 'مخصص',
          primary: initialEvent.theme.primary,
          secondary: initialEvent.theme.secondary,
          gradient: 'from-amber-400 to-rose-500',
        });
      }
      if (initialEvent.musicTrack) {
        const track = getTrackById(initialEvent.musicTrack);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-bold text-white leading-none tracking-tight">
                {initialEvent ? 'تعديل الدعوة ✏️' : 'إنشاء حدث جديد ✨'}
              </p>
              <p className="text-[10px] text-white/30 leading-none mt-0.5">
                {initialEvent ? 'قم بتعديل بيانات الدعوة والثيم والموسيقى' : 'اختر الثيم والموسيقى وأنشئ دعوتك'}
              </p>
            </div>
          </div>
          <Link href="/dashboard">
            ...
            <div className="luxury-card overflow-hidden">
              ...
              >
                ...
                >
                  ...
                  <h1 className="text-2xl font-bold">{initialEvent?.title || 'عنوان الحدث'}</h1>
                  <div className="w-16 h-0.5 bg-white/40 mx-auto rounded-full" />
                  <p className="text-white/70 text-sm">من تنظيم: {initialEvent?.hostName || 'مضيف الحدث'}</p>
...
export default function CreateEventPage() {
  return (
    <ProtectedRoute>
      <Suspense fallback={<div className="h-screen flex items-center justify-center text-white bg-[#060a14]">جاري التحميل...</div>}>
